...
users <- left_join(usersallinfo, papers %>% select(id_art, titre, abstract, Revue, NSFDisc, Nb_Page, Nb_Auteur, tweets, users, tweetspan), by = "id_art")
users$id_art <- as.character(as.numeric(users$id_art))
...
## =================
# 4) Clean the author descriptions for analysis
## =================
...
sci <- c("\\blectur(er|ing)\\b", 
         ...
         "\\bchair\\b", 
         "\\bassociate\\sdirector\\b", "\\b((micro)?bio|(e|é)co|pala?eonto|viro|onco|hema|epidemio|physio|archa?eo|anthropo|psycho|patho|endocrino|nephro|limno|socio|neuro|geo|physic|botan|entomo|hydro|genetic|chem|ethic)(log)?(u|ist|ien(ne)?)e?\\b", 
         "\\b(\\w+)?grad(\\w+)?\\b", 
         "\\borcid\\b", 
